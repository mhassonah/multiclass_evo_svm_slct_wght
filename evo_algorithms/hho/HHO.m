...
function [Rabbit_Energy,Rabbit_Location,CNVG]=HHO(N,T,lb,ub,dim,fobj, fitness_type, fs_run_type, trainLabel,trainData)
...
X=hho_initialization(N,dim,ub,lb)
...
while t<T
    for i=1:size(X,1)
        ...
        % fitness=fobj(X(i,:));
        fitness=fobj(X(i,:), fitness_type, fs_run_type, trainLabel,trainData);
    ...
    for i=1:size(X,1)
        ...
        elseif abs(Escaping_Energy)<1
            ...
            if r<0.5 && abs(Escaping_Energy)>=0.5, % Soft besiege % rabbit try to escape by many zigzag deceptive motions
                ...
                Xi_fitness = fobj(X(i,:), fitness_type, fs_run_type, trainLabel,trainData);
                
                if fobj(X1, fitness_type, fs_run_type, trainLabel,trainData)...
                        < Xi_fitness % improved move?
                    X(i,:)=X1;
                else % hawks perform levy-based short rapid dives around the rabbit
                    X2=Rabbit_Location-Escaping_Energy*abs(Jump_strength*Rabbit_Location-X(i,:))+rand(1,dim).*Levy(dim);
                    if (fobj(X2, fitness_type, fs_run_type, trainLabel,trainData)<Xi_fitness), % improved move?
                        X(i,:)=X2;
                    end
                end
            end
            ...
            if r<0.5 && abs(Escaping_Energy)<0.5, % Hard besiege % rabbit try to escape by many zigzag deceptive motions
                ...
                if fobj(X1, fitness_type, fs_run_type, trainLabel,trainData)... 
                    ...
                    if (fobj(X2, fitness_type, fs_run_type, trainLabel,trainData) ...
                            < Xi_fitness), % improved move?
                        X(i,:)=X2;
                    end
                end

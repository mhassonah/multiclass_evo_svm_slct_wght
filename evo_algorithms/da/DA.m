...
function [Best_score,Best_pos,cg_curve]=DA(SearchAgents_no,Max_iteration,lb,ub,dim,fobj, fitness_type, fs_run_type,trainLabel,trainData)
...
X=da_initialization(SearchAgents_no,dim,ub,lb);
Fitness=zeros(1,SearchAgents_no);

DeltaX=da_initialization(SearchAgents_no,dim,ub,lb);
...
for iter=1:Max_iteration
    ...
    for i=1:SearchAgents_no %Calculate all the objective values first
        Fitness(1,i)=fobj(X(:,i)', fitness_type, fs_run_type, trainLabel,trainData);
    ...
    for i=1:SearchAgents_no
        ...
        for j=1:SearchAgents_no
            Dist2Enemy=da_distance(X(:,i),X(:,j));
        ...
        % Attraction to food%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Eq. (3.4)
        Dist2Food=da_distance(X(:,i),Food_pos(:,1));
        if all(Dist2Food<=r)
            F=Food_pos-X(:,i);
        else
            F=0;
        ...
        % Eq. (3.5)
        Dist2Enemy=da_distance(X(:,i),Enemy_pos(:,1));
        if all(Dist2Enemy<=r)
            Enemy=Enemy_pos+X(:,i);
        else
            Enemy=zeros(dim,1);

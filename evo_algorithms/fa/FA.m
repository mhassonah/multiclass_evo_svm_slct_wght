...
function [bestsolution, convergence_curve] = FA (population, max_iteration, lb, ub, dim, fitness_type, fs_run_type, trainLabel,trainData)
% parameters [n N_iteration alpha betamin gamma]
para=[population max_iteration 0.5 0.2 1];
...
[u,fval,NumEval,cg_curve]=ffa_mincon(@get_fintess_value,u0,Lb,Ub,para, fitness_type, fs_run_type, trainLabel,trainData);
...
%% Cost or Objective function
 function z=get_fintess_value(x,fitness_type, trainLabel,trainData)
% Exact solutions should be (1,1,...,1) 
z=cost(x, fitness_type, trainLabel,trainData);
...
% Start FA
function [nbest,fbest,NumEval,cg_curve]...
           =ffa_mincon(fhandle,u0, Lb, Ub, para, fitness_type, fs_run_type, trainLabel,trainData)
...
cg_curve = zeros(1,MaxGeneration);
...
for i=1:n,
   zn(i)=fhandle(ns(i,:), fitness_type, fs_run_type, trainLabel,trainData);
   Lightn(i)=zn(i);
end

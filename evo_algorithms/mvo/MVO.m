...
function [Best_universe_Inflation_rate,Best_universe,Convergence_curve]= ...
    MVO(N,Max_time,lb,ub,dim,fobj, fitness_type, fs_run_type, ...
...
Universes=mvo_initialization(N,dim,ub,lb);
...
while Time<Max_time+1
    ...
    for i=1:size(Universes,1)
        ...
        Inflation_rates(1,i)=fobj(Universes(i,:), fitness_type, fs_run_type, trainLabel,trainData);
    ...
    for i=2:size(Universes,1)%Starting from 2 since the firt one is the elite
        ...
        for j=1:size(Universes,2)
            ...
            if r1<normalized_sorted_Inflation_rates(i)
                White_hole_index=mvo_RouletteWheelSelection(-sorted_Inflation_rates);% for maximization problem -sorted_Inflation_rates should be written as sorted_Inflation_rates

...
function [TargetFitness,TargetPosition,Convergence_curve,Trajectories,fitness_history, position_history]=GOA(N, Max_iter, lb,ub, dim, fobj, fitness_type, fs_run_type, trainLabel,trainData)

disp('GOA is not estimating the global optimum for your problem....')
...
GrassHopperPositions=goa_initialization(N,dim,ub,lb);
...
for i=1:size(GrassHopperPositions,1)
    GrassHopperFitness(1,i)=fobj(GrassHopperPositions(i,:), fitness_type, fs_run_type, trainLabel,trainData);
...
while l<Max_iter+1
    ...
    for i=1:size(GrassHopperPositions,1)
        ...
        for k=1:2:dim
            ...
            for j=1:N
                if i~=j
                    Dist=goa_distance(temp(k:k+1,j), temp(k:k+1,i)); % Calculate the distance between two grasshoppers
                    
                    r_ij_vec=(temp(k:k+1,j)-temp(k:k+1,i))/(Dist+eps); % xj-xi/dij in Eq. (2.7)
                    xj_xi=2+rem(Dist,2); % |xjd - xid| in Eq. (2.7) 
                    
                    s_ij=((ub(k:k+1) - lb(k:k+1))*c/2)*goa_S_func(xj_xi).*r_ij_vec; % The first part inside the big bracket in Eq. (2.7)
                    S_i=S_i+s_ij;
                end
            end
            S_i_total(k:k+1, :) = S_i;
